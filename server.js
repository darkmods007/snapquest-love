...
db.serialize(() => {
    ...
    console.log('✅ Database ready with Profiles & DarkMode support');
});
...
app.post('/api/auth/register', (req, res) => {
    ...
    db.run('INSERT INTO users (username, password, full_name) VALUES (?, ?, ?)', 
        ...
        function(err) {
            ...
            res.json({ user: { id: this.lastID, username, full_name: full_name || username, profile_pic: '' } });
        });
});
...
app.get('/api/me', (req, res) => {
    ...
    db.get('SELECT id, username, full_name, profile_pic FROM users WHERE id = ?', [req.session.userId], (err, user) => {
...
app.get('/api/photos', isAuthenticated, (req, res) => {
    ...
    // On JOIN pour avoir la photo de profil à jour de l'auteur de la publication
    const queryPhotos = `
        SELECT p.*, u.profile_pic as user_avatar, u.username as current_username, u.full_name as current_fullname
        FROM photos p LEFT JOIN users u ON p.user_id = u.id ORDER BY p.created_date DESC
    `;
    ...
    db.all(queryPhotos, [], (err, photos) => {
        ...
        db.all('SELECT * FROM likes', [], (err, allLikes) => {
            const queryComments = `
                ...
                FROM comments c LEFT JOIN users u ON c.user_id = u.id ORDER BY c.created_at ASC
            `;
...
app.post('/api/photos/:id/comments', isAuthenticated, (req, res) => {
    ...
    db.get('SELECT full_name FROM users WHERE id = ?', [req.session.userId], (err, user) => {
        db.run('INSERT INTO comments (photo_id, user_id, user_name, text) VALUES (?, ?, ?, ?)',
            [photoId, req.session.userId, user ? user.full_name : 'Anonyme', text], function(err) {
...
app.post('/api/messages', isAuthenticated, (req, res) => {
    ...
    db.get('SELECT full_name FROM users WHERE id = ?', [req.session.userId], (err, user) => {
        db.run('INSERT INTO global_messages (user_id, user_name, text) VALUES (?, ?, ?)',
            [req.session.userId, user ? user.full_name : 'Anonyme', text], function(err) {
                res.json({ success: true });
            });
    });
});
