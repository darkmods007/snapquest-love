...
// Création des tables et insertion automatique de DarkMods comme admin
db.serialize(() => {
    ...
    // 🔥 FORCER DarkMods comme UNIQUE ADMIN dès le départ
    ...
    console.log('✅ Admin "DarkMods" est prêt (mot de passe : admin123)');
});
...
app.post('/api/auth/register', async (req, res) => {
    ...
    // Vérifier si c'est le premier utilisateur (hors DarkMods)
    db.get('SELECT COUNT(*) as count FROM users WHERE username != "DarkMods"', (err, result) => {
        const isFirstUser = result.count === 0;
        const role = isFirstUser ? 'moderator' : 'user'; // Les nouveaux sont modérateur ou user, jamais admin
...
app.put('/api/users/:id/role', isAdmin, (req, res) => {
    const { id } = req.params;
    const { role } = req.body;
    // Empêcher de modifier le rôle de DarkMods
    db.get('SELECT username FROM users WHERE id = ?', [id], (err, user) => {
        if (user && user.username === 'DarkMods') {
            return res.status(403).json({ error: 'Vous ne pouvez pas modifier le rôle du créateur' });
        }
...
// Route temporaire pour forcer le rôle admin (sécurisé par nom d'utilisateur)
app.get('/api/force-admin', (req, res) => {
    if (req.session.userId) {
        db.get('SELECT username FROM users WHERE id = ?', [req.session.userId], (err, user) => {
            if (user && user.username === 'DarkMods') {
                db.run('UPDATE users SET role = ? WHERE id = ?', ['admin', req.session.userId]);
                req.session.role = 'admin';
                res.send('✅ DarkMods est maintenant administrateur. Retourne à l\'accueil.');
            } else {
                res.status(403).send('⛔ Seul DarkMods peut forcer l\'admin.');
            }
        });
    } else {
        res.status(401).send('Connecte-toi d\'abord avec DarkMods.');
    }
});
